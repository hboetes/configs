...
;; Time-stamp: <2004-11-08 13:58:28 michaelw>
...
;; Toggle CBox minor mode with M-x mwe:color-box-mode
;; Sit back.  Enjoy. :)

;; Notes: 
;;   * Buffer is made read-only, so edits are not possible while showing
;;     colors.
;;   * Abuses `undo', so don't try to change buffer in any way.
;;
;; Inspired by http://www.32768.com/bill/weblog/000660.shtml#000660

;;
;; As an example, uncomment FACTORIAL, mark it, and
;; use M-x `mwe:color-box-region/miscbill'.  To undo the coloring, use
;; M-x `mwe:clear-color-boxes'.
;;
...
;;;###autoload
(define-minor-mode mwe:color-box-mode
  "Display pretty boxes for each nesting level."
  nil " CBox" nil
  (if mwe:color-box-mode
      (mwe:color-box-buffer)
    (mwe:clear-color-boxes)))

;;;###autoload
(defun mwe:clear-color-boxes ()
  "Clear all color boxes in buffer.
See also `mwe:color-box-region'."
  (interactive)
  (toggle-read-only -1)
  (undo)
  (remove-text-properties (point-min) (point-max) '(invisible))
  (dolist (o *mwe:color-box-overlays*)
    (delete-overlay o))
  (setq *mwe:color-box-overlays* ()))
...
See also `mwe:color-box-region'."
  ...
  (with-current-buffer (or buf (current-buffer))
    (let
	((*mwe:region-tokenizer* #'mwe:slist-tokenizer))
...
(provide 'mwe-color-boxes)
;;; mwe-color-boxes.el ends here
